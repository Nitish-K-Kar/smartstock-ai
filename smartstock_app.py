...
<style>
  ...
  [data-testid="stAppViewContainer"] { background: #0E1A2B !important; }
  ...
  /* Tabs */
  div[data-testid="stTabs"] button { font-size: 0.9rem; color: #8DA4C4 !important; }
  div[data-testid="stTabs"] button[aria-selected="true"] { color: #00C9A7 !important; }
  /* Metric cards */
  .metric-card {
    background: #132030; border-radius: 12px; padding: 1.1rem 1.3rem;
    border: 0.5px solid #1E3A52; margin-bottom: 0;
  }
  .metric-card .val { font-size: 2rem; font-weight: 600; margin: 0; }
  .metric-card .lbl { font-size: 0.78rem; color: #8DA4C4; margin: 0 0 4px; }
  /* Badges */
  .badge { display: inline-block; padding: 2px 10px; border-radius: 20px; font-size: 0.72rem; font-weight: 600; }
  .badge-critical { background:#4A1212; color:#FF8A8A; }
  .badge-medium   { background:#3D2A00; color:#FFD07A; }
  .badge-safe     { background:#0A2E20; color:#4EEAB4; }
  /* Header */
  .header-bar {
    background: #0A1628; border-radius: 12px; padding: 1rem 1.5rem;
    margin-bottom: 1.2rem; display: flex; align-items: center; gap: 12px;
    border: 0.5px solid #1E3A52;
  }
  /* AI insight box */
  .insight-box {
    background: #0A2E20; border: 1px solid #1D9E75;
    border-radius: 10px; padding: 1rem 1.2rem; margin-top: 0.8rem;
  }
  .insight-box div { color: #B8F0DC !important; }
  /* Section titles */
  .section-title { font-size: 0.85rem; font-weight: 600; color: #00C9A7;
  ...
  [data-testid="stMetricValue"] { color: #D8E6F3 !important; }
  [data-testid="stMetricLabel"] { color: #8DA4C4 !important; }
  /* Captions */
  [data-testid="stCaptionContainer"] p { color: #607090 !important; }
</style>
...
with tab1:
    ...
    with col_chart:
        ...
        fig_bar.update_layout(
            ...
            plot_bgcolor="#132030", paper_bgcolor="#0E1A2B",
            showlegend=False, yaxis=dict(showgrid=True, gridcolor="#1E3A52", color="#8DA4C4"),
            xaxis=dict(showgrid=False, color="#8DA4C4"), font=dict(color="#D8E6F3"),
        )
...
with tab3:
    if "selected_sku" not in st.session_state:
        ...
        fig.update_layout(
            ...
            plot_bgcolor="#132030", paper_bgcolor="#0E1A2B",
            legend=dict(orientation="h", y=-0.18, font=dict(size=11, color="#8DA4C4")),
            xaxis=dict(showgrid=False, color="#8DA4C4"),
            yaxis=dict(showgrid=True, gridcolor="#1E3A52", title="Units / month", color="#8DA4C4"),
            font=dict(color="#D8E6F3"),
            hovermode="x unified",
        )
...
Be direct, data-driven, and actionable. Max 2 sentences each."""
            ...
            with st.spinner("Claude AI is analyzing this SKU..."):
                try:
                    ...
                    # Smart fallback — generates a realistic insight from the data without needing the API
                    days = row['days_left']
                    lt   = row['lead_time_days']
                    qty  = int(row['reorder_qty'])
                    stk  = int(row['current_stock'])
                    ds   = row['avg_daily_sales']
                    cover_after = round((qty + stk) / ds) if ds else 0

                    if risk == 'critical':
                        situation = (f"• Situation: {row['name']} has only {stk} units remaining — just {days:.0f} days of cover "
                                     f"against a {lt}-day supplier lead time. A stockout is imminent without immediate action.")
                        key_risk  = (f"• Risk: Orders placed today will arrive {lt - days:.0f} days after stock is exhausted, "
                                     f"creating a gap that directly translates to lost sales and dissatisfied customers.")
                        action    = (f"• Action: Raise an emergency PO for {qty} units today. "
                                     f"This restores {cover_after} days of cover and eliminates the stockout window.")
                    elif risk == 'medium':
                        situation = (f"• Situation: {row['name']} holds {stk} units — {days:.0f} days of cover against "
                                     f"a {lt}-day lead time. Stock is within the reorder buffer zone.")
                        key_risk  = (f"• Risk: Any demand spike above {ds:.1f} units/day or a supplier delay beyond {lt} days "
                                     f"will push this SKU into critical territory within 2 weeks.")
                        action    = (f"• Action: Place a planned PO for {qty} units within 3 days. "
                                     f"This brings cover to {cover_after} days — safely above the reorder threshold.")
                    else:
                        situation = (f"• Situation: {row['name']} is well-stocked at {stk} units, "
                                     f"providing {days:.0f} days of cover — more than double the {lt}-day lead time.")
                        key_risk  = (f"• Risk: Over-stocking risk. Holding excess inventory ties up working capital "
                                     f"and increases warehousing and obsolescence costs.")
                        action    = (f"• Action: No reorder needed for at least {int(days - lt * 1.5):.0f} days. "
                                     f"Monitor weekly; set a reorder alert at {int(ds * lt * 1.5)} units.")

                    st.session_state[f"insight_{sku_id}"] = f"{situation}\n\n{key_risk}\n\n{action}"
